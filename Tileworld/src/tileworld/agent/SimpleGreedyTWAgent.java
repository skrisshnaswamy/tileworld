...
//        this.id = id;
        this.name = name;
        System.out.println("Agent @ (" + xpos + "," + ypos + ")");
//        findFuelingStation();
//        this.setPatrol();
    ...
    private void computeFuelLimits() {
        ...
        if (path != null & path.getpath() != null) {
            ...
            this.FUEL_LOW = (int) this.getFuelLevel() / 5;
            this.FUEL_CRITICAL = (int) this.getFuelLevel() / 7;
        }
        System.out.println("*********************************************");
        System.out.println("Fuel limit set to Low: " + this.FUEL_LOW + ", Critical: " + this.FUEL_CRITICAL);
        System.out.println("*********************************************");
    }
...
//            System.out.println("Looking at (" + (this.getX() + dir.dx) + "," + (this.getY() + dir.dy) + ")");
            cellNotBlocked = false;
        }
//        System.out.print("isMovePossible: ");
//        System.out.println(withinBoundary && extraCond && cellNotBlocked);
            ...
            for (int y0 = ymin; y0 <= ymax; ++y0) {
                ...
                if (obj && (e instanceof TWFuelStation)) {
                    if (!memcoords.contains(x0 + ":" + y0)) {
                        System.out.println("Fueling station found at (" + x0 + "," + y0 + ")");
                            memcoords.add(x0 + ":" + y0);
    ...
    protected TWThought think() {
        ...
        ArrayList<Message> msgs = this.getEnvironment().getMessages();

        isInvalidCoords();
        if (!isFuelingStnLocKnown) {
            System.out.println("Searching fueling station...; Current fuel level:" + this.getFuelLevel());
        ...
        // whenever a broadcasted msg on fuelstation location is received, push to memory
...
//                    int x = Integer.parseInt(splitMsg[1]);
//                    int y = Integer.parseInt(splitMsg[2]);
//                    if (!memcoords.contains(x + ":" + y)) {
//                        memcoords.add(x + ":" + y);
...
//        System.out.println("YIncrement:" + yIncrement);
        System.out.println("*************************************************************");
        System.out.println("Starting at (" + this.getX() + "," + this.getY() + "), Fuel level: " + this.getFuelLevel());
        System.out.println("Closest Corner:" + closestCorner.toString());
        boolean fuelFound = locateFuelingStation();
        if (fuelFound || checkMsgForFuelStation())
            return true;
        System.out.println("Moving to closest Corner:" + closestCorner.toString());
        if (fpath != null && fpath.getpath() != null && fpath.getpath().size() > 0) {
            for (TWPathStep eachStep : fpath.getpath()) {
                try {
                    ...
                    fuelFound = locateFuelingStation();
                    if (fuelFound || checkMsgForFuelStation())
                        return true;
        ...
        for (int i = 0; i < yIter; i++) {
            ...
            System.out.print("Sideways sweep complete...");
            System.out.println("Currently @ (" + this.getX() + "," + this.getY() + "), Fuel level: " + this.getFuelLevel());
...
//                    System.out.println("Looking at (" + x0 + "," + y0 + ") in memory");
    ...
    private void emergencyRefueling() {
        System.out.println("Rushing for emergency refueling...");
        TWPath npath;
        ObjectGrid2D objectGrid = this.getEnvironment().getObjectGrid();
        int x = 0, y = 0;
        ...
        if (fStn == null) {
            System.out.println("Fuel not in memory! checking msgs...");
        ...
        if (fuelFound) {
            ...
            if (fStnLocLocked) {
                System.out.println("Action: Refueling!!!!!!!!");
                this.refuel();
            }
        }
    }
